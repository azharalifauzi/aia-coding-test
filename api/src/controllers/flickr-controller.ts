...
export const getPhotos = async (req: Request, res: Response) => {
  ...
  let flickrUrl = `https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=${process.env.FLICKR_API_KEY}&privacy_filter=1&media=photos&per_page=${limit}&page=${page}&format=json&nojsoncallback=1`;
  ...
  res.send({
    ...
    data: {
      ...
      photos: Array.from(flickrData.photos.photo || [], ({ id, title, server, secret }) => ({
        id,
        title,
